...
from st13_survey_utils import (  # noqa: E402
    bootstrap_erreygers,
    estimate_by_group,
    estimate_prop,
    format_pct_ci,
...
def _research_root() -> Path:
    """Locate the Research root portably (Windows or WSL)."""
    here = Path(__file__).resolve()
    for parent in here.parents:
        if (parent / "01_DHS_Data").is_dir() or (parent / "Kenya - STEPS 2015").is_dir():
            return parent
    for cand in (Path("C:/Users/HFD 2/Research"), Path("/mnt/c/Users/HFD 2/Research")):
        if cand.is_dir():
            return cand
    raise FileNotFoundError("Could not locate the Research root")


RESEARCH_ROOT = _research_root()

STEPS_CSV = RESEARCH_ROOT / "Kenya - STEPS 2015" / "ken2015.csv"
...
def construct(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["told_high_bp"] = yes1(d["h2a"])  # ever told; asked only if h1 == 1
    # ST13: the 12-month variant (h2b, asked only if h2a == 1). Mohamed et al.
    # (BMC Public Health 2018) used this narrower definition on the same data;
    # reporting both is what lets ST13 reconcile with the published national
    # cascade instead of silently diverging from it. Respondents never told
    # (h2a != 1) or never measured (h1 != 1) are not-aware, not missing.
    d["told_high_bp_12m"] = yes1(d["h2b"])
    d.loc[d["told_high_bp"] != 1, "told_high_bp_12m"] = 0.0
    ...
    # ST13: awareness on the narrower 12-month definition, for reconciliation
    # with the published national cascade. Note that medication adds nothing to
    # either definition in these data -- every respondent on treatment had also
    # been told -- so `treated` is a subset of `aware` empirically as well as by
    # clinical logic, under any definition.
    d["htn_aware_12m"] = np.where(
        d["htn_measured"] == 1,
        ((d["told_high_bp_12m"] == 1) | (d["on_bp_meds_pop"] == 1)).astype(float),
        np.nan,
    )
...
def main():
    ...
    d.to_pickle(OUT_DERIVED / "st13_steps_analytic.pkl")
    ...
    d[keep].to_csv(OUT_DERIVED / "st13_steps_analytic_slim.csv", index=False)
